...
function CDMOptions.ApplyIconConfigUpdate(buttonFrame, buttonData, anchorIndex, mode, isGlobal, isBuffBar, refreshStates)
	if buttonFrame.data.isCustom then
		SCM:CreateAllCustomIcons(buttonData.iconType)
		SCM:ApplyAnchorGroupCDManagerConfig(anchorIndex, isGlobal, nil, refreshStates)
		return
	end
	Options.ApplyModeConfigUpdate(anchorIndex, mode, refreshStates)
end
