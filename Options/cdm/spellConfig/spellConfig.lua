...
function CDMOptions.ApplyIconConfigUpdate(buttonFrame, buttonData, anchorIndex, mode, isGlobal, isBuffBar, refreshOptions, refreshGlowOptions)
	if buttonFrame.data.isCustom then
		SCM:CreateAllCustomIcons(buttonData.iconType)
		SCM:ApplyAnchorGroupCDManagerConfig(anchorIndex, isGlobal, nil, refreshOptions, refreshGlowOptions)
		return
	end
	Options.ApplyModeConfigUpdate(anchorIndex, mode, refreshOptions, refreshGlowOptions)
end
